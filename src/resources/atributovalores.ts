...
export class AtributoValoresResource {
  ...
  async getResource(uri: string): Promise<Resource> {
    ...
    try {
      const result = await this.client.getWithPagination<AtributoValor>(
        '/atributovalores',
        limit,
        offset,
        additionalParams
      ...
      return {
        uri,
        name: 'FacturaScripts Attribute Values',
      ...
      return {
        uri,
        name: 'FacturaScripts Attribute Values (Error)',
        mimeType: 'application/json',
        contents: [
          {
            type: 'text',
  ...
  matchesUri(uri: string): boolean {
    try {
      const url = new URL(uri);
      return url.protocol === 'facturascripts:' && url.hostname === 'atributovalores';
    } catch {
      return false;
    }
  }
