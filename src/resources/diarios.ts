...
import { Diario } from '../types/facturascripts.js';
import { parseUrlParameters } from '../utils/filterParser.js';

export class DiariosResource {
  ...
  async getResource(uri: string): Promise<Resource> {
    ...
    try {
      const result = await this.client.getWithPagination<Diario>(
        '/diarios',
        limit,
        offset,
        additionalParams
      ...
      return {
        uri,
        name: 'FacturaScripts Diarios (Error)',
        mimeType: 'application/json',
        contents: [
          {
            type: 'text',
  ...
  matchesUri(uri: string): boolean {
    try {
      ...
      return url.protocol === 'facturascripts:' && url.hostname === 'diarios';
    } catch {
      return false;
    }
  }
