...
import { Empresa } from '../types/facturascripts.js';

export class EmpresasResource {
  ...
  async getResource(uri: string): Promise<Resource> {
    ...
    try {
      const result = await this.client.getWithPagination<Empresa>(
        '/empresas',
        limit,
        offset,
        additionalParams
      ...
      return {
        uri,
        name: 'FacturaScripts Empresas (Error)',
        mimeType: 'application/json',
        contents: [
          {
            type: 'text',
  ...
  matchesUri(uri: string): boolean {
    try {
      const url = new URL(uri);
      return url.protocol === 'facturascripts:' && url.hostname === 'empresas';
    } catch {
      return false;
    }
  }
