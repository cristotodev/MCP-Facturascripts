...
import { CronJob } from './resource.js';

export const cronjobesTool: Tool = {
  name: 'get_cronjobes',
  description: 'Obtener tareas programadas (cron jobs) con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleCronjobesTool(
  ...
  try {
    const result = await client.getWithPagination<CronJob>(
      '/cronjobes',
      limit,
      offset,
      additionalParams
