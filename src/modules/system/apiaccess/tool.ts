...
import { ApiAccess } from '../../../types/facturascripts.js';

export const apiaccessTool: Tool = {
  name: 'get_apiaccess',
  description: 'Obtener configuración de acceso API con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleApiaccessTool(
  ...
  try {
    const result = await client.getWithPagination<ApiAccess>(
      '/apiaccess',
      limit,
      offset,
      additionalParams
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              error: 'Failed to fetch apiaccess',
