...
export const attachedfilerelationsTool: Tool = {
  name: 'get_attachedfilerelations',
  description: 'Obtener relaciones de archivos adjuntos con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleAttachedfilerelationsTool(
  ...
  try {
    const result = await client.getWithPagination<AttachedFileRelation>(
