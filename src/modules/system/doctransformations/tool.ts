...
import { DocTransformation } from '../../../types/facturascripts.js';

export const doctransformationsTool: Tool = {
  name: 'get_doctransformations',
  description: 'Obtener transformaciones de documentos con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleDoctransformationsTool(
  ...
  try {
    const result = await client.getWithPagination<DocTransformation>(
      '/doctransformations',
      limit,
      offset,
      additionalParams
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              error: 'Failed to fetch doctransformations',
