...
import type { LineaFacturaProveedor } from '../../../../types/facturascripts.js';

export const toolDefinition = {
  name: 'get_lineafacturaproveedores',
  description: 'Obtiene la lista de líneas de facturas de proveedores con paginación y filtros avanzados',
  inputSchema: {
    type: 'object',
    properties: {
...
export async function toolImplementation(
  ...
  const uri = `facturascripts://lineafacturaproveedores?${params.toString()}`;
  const { additionalParams } = parseUrlParameters(uri);

  try {
    const result = await client.getWithPagination<LineaFacturaProveedor>(
