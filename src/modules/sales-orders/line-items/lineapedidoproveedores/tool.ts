...
import type { LineaPedidoProveedor } from '../../../../types/facturascripts.js';

export const toolDefinition = {
  name: 'get_lineapedidoproveedores',
  description: 'Obtiene la lista de líneas de pedidos de proveedores con paginación y filtros avanzados',
  inputSchema: {
    type: 'object',
    properties: {
...
export async function toolImplementation(
  ...
  const uri = `facturascripts://lineapedidoproveedores?${params.toString()}`;
  const { additionalParams } = parseUrlParameters(uri);

  try {
    const result = await client.getWithPagination<LineaPedidoProveedor>(
