...
import type { LineaPedidoCliente } from '../../../../types/facturascripts.js';

export const toolDefinition = {
  name: 'get_lineapedidoclientes',
  description: 'Obtiene la lista de líneas de pedidos de clientes con paginación y filtros avanzados',
  inputSchema: {
    type: 'object',
    properties: {
      ...
      filter: { type: 'string', description: 'Filtros en formato campo:valor (ej: idpedido:123)', default: '' },
...
export async function toolImplementation(
  ...
  const uri = `facturascripts://lineapedidoclientes?${params.toString()}`;
  const { additionalParams } = parseUrlParameters(uri);

  try {
    const result = await client.getWithPagination<LineaPedidoCliente>(
      '/lineapedidoclientes',
      limit,
      offset,
      additionalParams
      ...
      content: [
        {
          type: 'text' as const,
          text: JSON.stringify({
            error: 'Failed to fetch lineapedidoclientes',
