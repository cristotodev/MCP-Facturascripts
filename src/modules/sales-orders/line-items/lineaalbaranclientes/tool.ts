...
import type { LineaAlbaranCliente } from '../../../../types/facturascripts.js';

export const toolDefinition = {
  name: 'get_lineaalbaranclientes',
  description: 'Obtiene la lista de líneas de albaranes de clientes con paginación y filtros avanzados',
  inputSchema: {
    type: 'object',
    properties: {
...
export async function toolImplementation(
  ...
  const uri = `facturascripts://lineaalbaranclientes?${params.toString()}`;
  const { additionalParams } = parseUrlParameters(uri);

  try {
    const result = await client.getWithPagination<LineaAlbaranCliente>(
