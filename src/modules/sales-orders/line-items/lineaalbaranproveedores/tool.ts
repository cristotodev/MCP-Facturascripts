...
import type { LineaAlbaranProveedor } from '../../../../types/facturascripts.js';

export const toolDefinition = {
  name: 'get_lineaalbaranproveedores',
  description: 'Obtiene la lista de líneas de albaranes de proveedores con paginación y filtros avanzados',
  inputSchema: {
    type: 'object',
    properties: {
      ...
      filter: { type: 'string', description: 'Filtros en formato campo:valor (ej: idalbaran:123)', default: '' },
...
export async function toolImplementation(
  ...
  const uri = `facturascripts://lineaalbaranproveedores?${params.toString()}`;
  const { additionalParams } = parseUrlParameters(uri);

  try {
    const result = await client.getWithPagination<LineaAlbaranProveedor>(
      ...
      content: [
        {
          type: 'text' as const,
          text: JSON.stringify({
            error: 'Failed to fetch lineaalbaranproveedores',
