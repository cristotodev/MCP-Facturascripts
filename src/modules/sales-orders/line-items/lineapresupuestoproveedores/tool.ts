...
import type { LineaPresupuestoProveedor } from '../../../../types/facturascripts.js';

export const toolDefinition = {
  name: 'get_lineapresupuestoproveedores',
  description: 'Obtiene la lista de líneas de presupuestos de proveedores con paginación y filtros avanzados',
  inputSchema: {
    type: 'object',
    properties: {
...
export async function toolImplementation(
  ...
  const uri = `facturascripts://lineapresupuestoproveedores?${params.toString()}`;
  const { additionalParams } = parseUrlParameters(uri);

  try {
    const result = await client.getWithPagination<LineaPresupuestoProveedor>(
