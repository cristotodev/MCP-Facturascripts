...
import type { AlbaranCliente } from './resource.js';
...
export async function toolImplementation(
  ...
  const uri = `facturascripts://albaranclientes?${params.toString()}`;
  const { additionalParams } = parseUrlParameters(uri);

  try {
    const result = await client.getWithPagination<AlbaranCliente>(
      '/albaranclientes',
      limit,
      offset,
      additionalParams
