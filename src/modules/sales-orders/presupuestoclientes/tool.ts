...
import type { PresupuestoCliente } from './resource.js';
...
export async function toolImplementation(
  ...
  const uri = `facturascripts://presupuestoclientes?${params.toString()}`;
  const { additionalParams } = parseUrlParameters(uri);

  try {
    const result = await client.getWithPagination<PresupuestoCliente>(
      '/presupuestoclientes',
      limit,
      offset,
      additionalParams
      ...
      content: [
        {
          type: 'text' as const,
          text: JSON.stringify({
            error: 'Failed to fetch presupuestoclientes',
