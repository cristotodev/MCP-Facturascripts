...
import { AgenciaTransporte } from './resource.js';

export const agenciatransportesTool: Tool = {
  name: 'get_agenciatransportes',
  description: 'Obtener agencias de transporte con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleAgenciatransportesTool(
  ...
  try {
    const result = await client.getWithPagination<AgenciaTransporte>(
      '/agenciatransportes',
      limit,
      offset,
      additionalParams
