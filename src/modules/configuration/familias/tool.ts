...
import { Familia } from '../../../types/facturascripts.js';

export const familiasTool: Tool = {
  name: 'get_familias',
  description: 'Obtener familias de productos con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleFamiliasTool(
  ...
  try {
    const result = await client.getWithPagination<Familia>(
      '/familias',
      limit,
      offset,
      additionalParams
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              error: 'Failed to fetch familias',
