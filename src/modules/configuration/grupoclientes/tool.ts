...
import { GrupoClientes } from '../../../types/facturascripts.js';

export const grupoclientesTool: Tool = {
  name: 'get_grupoclientes',
  description: 'Obtener grupos de clientes con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleGrupoclientesTool(
  ...
  try {
    const result = await client.getWithPagination<GrupoClientes>(
      '/grupoclientes',
      limit,
      offset,
      additionalParams
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              error: 'Failed to fetch grupoclientes',
