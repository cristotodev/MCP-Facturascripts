...
import { IdentificadorFiscal } from '../../../types/facturascripts.js';

export const identificadorfiscalesTool: Tool = {
  name: 'get_identificadorfiscales',
  description: 'Obtener identificadores fiscales con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleIdentificadorfiscalesTool(
  ...
  try {
    const result = await client.getWithPagination<IdentificadorFiscal>(
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              error: 'Failed to fetch identificadorfiscales',
