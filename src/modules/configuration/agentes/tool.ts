...
import { Agente } from './resource.js';

export const agentesTool: Tool = {
  name: 'get_agentes',
  description: 'Obtener agentes comerciales con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleAgentesTool(
  ...
  try {
    const result = await client.getWithPagination<Agente>(
      '/agentes',
      limit,
      offset,
      additionalParams
