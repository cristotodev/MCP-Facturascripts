...
import { Almacen } from './resource.js';

export const almacenesTool: Tool = {
  name: 'get_almacenes',
  description: 'Obtener almacenes con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleAlmacenesTool(
  ...
  try {
    const result = await client.getWithPagination<Almacen>(
      '/almacenes',
      limit,
      offset,
      additionalParams
