...
import { EstadoDocumento } from '../../../types/facturascripts.js';

export const estadodocumentosTool: Tool = {
  name: 'get_estadodocumentos',
  description: 'Obtener estados de documentos con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleEstadodocumentosTool(
  ...
  try {
    const result = await client.getWithPagination<EstadoDocumento>(
      '/estadodocumentos',
      limit,
      offset,
      additionalParams
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              error: 'Failed to fetch estadodocumentos',
