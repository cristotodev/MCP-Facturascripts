...
import { FormaPago } from '../../../types/facturascripts.js';

export const formapagosTool: Tool = {
  name: 'get_formapagos',
  description: 'Obtener formas de pago con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleFormaPagosTool(
  ...
  try {
    const result = await client.getWithPagination<FormaPago>(
      '/formapagos',
      limit,
      offset,
      additionalParams
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              error: 'Failed to fetch formapagos',
