...
import { EmailSent } from '../../../types/facturascripts.js';

export const emailsentesTool: Tool = {
  name: 'get_emailsentes',
  description: 'Obtener emails enviados con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleEmailsentesTool(
  ...
  try {
    const result = await client.getWithPagination<EmailSent>(
      '/emailsentes',
      limit,
      offset,
      additionalParams
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              error: 'Failed to fetch emailsentes',
