...
import { EmailNotification } from '../../../types/facturascripts.js';

export const emailnotificationsTool: Tool = {
  name: 'get_emailnotifications',
  description: 'Obtener notificaciones por email con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleEmailnotificationsTool(
  ...
  try {
    const result = await client.getWithPagination<EmailNotification>(
      '/emailnotifications',
      limit,
      offset,
      additionalParams
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              error: 'Failed to fetch emailnotifications',
