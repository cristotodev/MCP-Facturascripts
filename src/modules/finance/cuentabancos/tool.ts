...
import type { CuentaBanco } from '../../../types/facturascripts.js';

export const cuentaBancosTool: Tool = {
  name: 'get_cuentabancos',
  description: 'Obtener cuentas bancarias con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
      ...
      filter: {
        type: 'string',
        description: 'Filtros en formato campo:valor (ej: codcuenta:5720001)',
      },
      order: {
        type: 'string',
...
export async function handleCuentaBancosCall(
  ...
  try {
    ...
    const result = await client.getWithPagination<CuentaBanco>(
      '/cuentabancos',
      limit,
      offset,
      additionalParams
