export const toolDefinition = {
  ...
  description: 'Obtiene la lista de recibos de proveedores con filtrado y paginación dinámicos',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function toolImplementation(args: any, client: any, buildUri: any) {
  const uri = buildUri('reciboproveedores', args);
  const { ReciboproveedoresResource } = await import('./resource.js');
  const resource = new ReciboproveedoresResource(client);
