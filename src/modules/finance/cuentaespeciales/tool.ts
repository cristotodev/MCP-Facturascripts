...
export const cuentaespecialesTool: Tool = {
  name: 'get_cuentaespeciales',
  description: 'Obtener cuentas especiales del plan contable con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleCuentaespecialesTool(
  ...
  try {
    const result = await client.getWithPagination<CuentaEspecial>(
      '/cuentaespeciales',
      limit,
      offset,
      additionalParams
