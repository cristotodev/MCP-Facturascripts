...
import { CuentaBancoProveedor } from '../../../types/facturascripts.js';

export const cuentabancoproveedoresTool: Tool = {
  name: 'get_cuentabancoproveedores',
  description: 'Obtener cuentas bancarias de proveedores con paginación y filtros',
  inputSchema: {
    type: 'object',
    properties: {
      limit: {
...
export async function handleCuentabancoproveedoresTool(
  ...
  try {
    const result = await client.getWithPagination<CuentaBancoProveedor>(
      ...
      content: [
        {
          ...
          text: JSON.stringify(
            {
              error: 'Failed to fetch cuentabancoproveedores',
