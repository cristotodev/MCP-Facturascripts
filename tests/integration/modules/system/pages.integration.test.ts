...
describe.skipIf(!shouldRunIntegrationTests)('Pages Integration Tests', () => {
  let client: FacturaScriptsClient;
  let pagesResource: PagesResource;

  beforeAll(() => {
    client = new FacturaScriptsClient();
    pagesResource = new PagesResource(client);
  });

  it('should fetch pages from real API', async () => {
    const result = await pagesResource.getResource('facturascripts://pages?limit=5');

    expect(result.uri).toBe('facturascripts://pages?limit=5');
    // Accept both success and error cases
    expect(result.name).toMatch(/^FacturaScripts Pages( \(Error\))?$/);
