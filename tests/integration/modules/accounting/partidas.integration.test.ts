...
describe.skipIf(!shouldRunIntegrationTests)('Partidas Integration Tests', () => {
  let client: FacturaScriptsClient;
  let partidasResource: PartidasResource;

  beforeAll(() => {
    client = new FacturaScriptsClient();
    partidasResource = new PartidasResource(client);
  });

  it('should fetch accounting entry lines from real API', async () => {
    const result = await partidasResource.getResource('facturascripts://partidas?limit=5');

    expect(result.uri).toBe('facturascripts://partidas?limit=5');
    // Accept both success and error cases
    expect(result.name).toMatch(/^FacturaScripts Accounting Entry Lines( \(Error\))?$/);
