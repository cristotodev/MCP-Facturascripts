...
describe.skipIf(!shouldRunIntegrationTests)('Countries Integration Tests', () => {
  let client: FacturaScriptsClient;
  let paisResource: PaisResource;

  beforeAll(() => {
    client = new FacturaScriptsClient();
    paisResource = new PaisResource(client);
  });

  it('should fetch countries from real API', async () => {
    const result = await paisResource.getResource('facturascripts://pais?limit=5');

    expect(result.uri).toBe('facturascripts://pais?limit=5');
    // Accept both success and error cases
    expect(result.name).toMatch(/^FacturaScripts Countries( \(Error\))?$/);
