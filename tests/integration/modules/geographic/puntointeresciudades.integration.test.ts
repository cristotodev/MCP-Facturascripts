...
describe.skipIf(!shouldRunIntegrationTests)('City Points of Interest Integration Tests', () => {
  ...
  beforeAll(() => {
    ...
    puntointeresciudadesResource = new PuntointeresciudadesResource(client);
  });

  it('should fetch city points of interest from real API', async () => {
    const result = await puntointeresciudadesResource.getResource('facturascripts://puntointeresciudades?limit=5');

    expect(result.uri).toBe('facturascripts://puntointeresciudades?limit=5');
    // Accept both success and error cases
    expect(result.name).toMatch(/^FacturaScripts City Points of Interest( \(Error\))?$/);
