...
import { CuentabancoproveedoresResource } from '../../../../src/modules/finance/cuentabancoproveedores/resource.js';
...
describe('CuentabancoproveedoresResource', () => {
  let resource: CuentabancoproveedoresResource;

  beforeEach(() => {
    resource = new CuentabancoproveedoresResource(mockClient);
    vi.clearAllMocks();
  });

  describe('matchesUri', () => {
    it('should return true for valid cuentabancoproveedores URI', () => {
      const result = resource.matchesUri('facturascripts://cuentabancoproveedores');
  ...
  describe('getResource', () => {
    it('should return formatted resource with default parameters', async () => {
      const mockData = [{ codproveedor: 'PROV001', iban: 'ES456', descripcion: 'Supplier account' }];
      ...
      const result = await resource.getResource('facturascripts://cuentabancoproveedores');
      
      expect(mockClient.getWithPagination).toHaveBeenCalledWith('/cuentabancoproveedores', 50, 0, {});
      expect(result.name).toBe('FacturaScripts CuentaBancoProveedores');
    ...
    it('should handle API errors gracefully', async () => {
      ...
      expect(result.name).toBe('FacturaScripts CuentaBancoProveedores (Error)');
      const content = JSON.parse(result.contents[0].text);
      expect(content.error).toBe('Failed to fetch cuentabancoproveedores');
